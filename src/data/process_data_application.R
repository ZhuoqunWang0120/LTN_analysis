...
for (i in seq_along(dietcovariates)){
  ...
  if (sum(is.na(Xadjust))==0 & det(t(Xadjust)%*%Xadjust)!=0){
    saveRDS(list(cnt=cnt,Xtest=Xtest,Xadjust=Xadjust,grouplabel=grouplabel,g=g,tree=tree),paste0(WORK_DIR,'/cache/application_',covariate,'.RData'))
  } 
...
if (sum(is.na(Xadjust))==0 & det(t(Xadjust)%*%Xadjust)!=0){
  saveRDS(list(cnt=cnt,Xtest=Xtest,Xadjust=Xadjust,grouplabel=grouplabel,g=g,tree=tree),paste0(WORK_DIR,'/cache/application_','casecontrol','.RData'))
} 
