...
# r -- pre-specified number of factors
# YL, Y -- N*p, pre-order
...
gibbs_crossgroup = function(N,
  ...
  if (reff) {
    if (reffcov == 1) {
      ...
      PHI_GAM = NULL
      omega2 = rWishart(1, p + 2, diag(p))[, , 1]
      OMEGA2 = list(omega2)
      if (lambda_fixed>0){
        lam2 = lambda_fixed
      }else{
      ...
      for (l in 2:p) {
        for (k in 1:(l - 1)) {
          ...
          ukl = rinvgauss(1, mu_prime, lam2 ^ 2)
          tau2[k, l] = 1 / ukl
          tau2[l, k] = 1 / ukl
        }
      }
  ...
  A2=NULL
  if (r > 0) {
    alpha = matrix(rnorm(r * p), r, p)
    ALPHA = list(alpha)
    delta = rgamma(r, 2, 1)
    tau = cumprod(delta)
    TAU = tau
    phi = matrix(rgamma(r * p, nu / 2, nu / 2), r, p) # r*p
    PHI = list(phi)
    if (a1a2=='hp'){
      a1 = rgamma(1,2, 1)
      a2 = rgamma(1,2, 1)
  ...
  aj = 1
  mj = 1 - (pnull) ^ (1 / p)
  pi1 = matrix(rbeta(q1 * p,aj*mj,aj*(1-mj)), q1, p)
  PI1 = list(pi1) # q1*p inclusion probability of beta1
  t0 = 1
  u0 = 0.001
  phi_pi = rgamma(q1, t0, u0)
  PHI_PI = phi_pi
  # sampling
  ACC1 = rep(0, niter)
  ACC2 = rep(0, niter)
  for (it in 2:niter) {
    ...
    if (reff) {
      ...
      if (reffcov == 1) {
        ...
        tau2 = omegatau[[2]]
        if (lambda_fixed<=0){
          lam2 = rgamma(1, 1 + p * (p + 1) / 2, 0.01 + exp(logsumexp(log(abs(omega2)))-log(2)))
        }
        OMEGA2[[it]] = omega2
        LAM2[[it]] = lam2
      }
    ...
    if (r > 0) {
      ...
      delta[1] = rgamma(1, a1 + p * r / 2, gampara2)
      # update delta 2-r
      if (r > 1) {
        for (h in 2:r) {
          ...
          gampara2 = 1 + s / 2
          delta[h] = rgamma(1, a2 + p * (r - h + 1) / 2, gampara2)
        }
      }
      # tau = cumprod(delta)
      ...
      if (a1a2=='hp'){
        at = rgamma(1, 2, 1) # choice of hyperparameter is from Dunson 2011
    ...
    for (k in 1:p) {
      for (j in 1:q1) {
        ...
        i0 = rbinom(1, 1, prob0) # indicator of 0
        if (i0 == 1) {
          beta1[j, k] = 0
          pi1[j, k] = 0
          pi1[j, k] = rbeta(1, aj * mj, aj * (1 - mj) + 1)
        } else {
          beta1[j, k] = rnorm(1, b1 * s1 ^ 2, s1) # rnorm(n,mean,SD), rmvnorm(n,mean,var)
          pi1[j, k] = rbeta(1, aj * mj + 1, aj * (1 - mj))
        }
      }
    }
    BETA1[[it]] = beta1
    ...
    beta2 = t(rmvnorm(p, rep(0, q2), invXTX)) %*% diag(1 / sqrt(phi_eps +
                                                                  1 / N / gprior_m)) + m # q2*p
    ...
    # phi_pi = mapply(function(x, y) { rgamma(1, x, y) }, t0 + nj_prime / 2, u0 + rowSums(beta1 ^ 2) / 2)
    ...
    phi_pi = mapply(function(x, y) { rgamma(1, x, y) }, t0 + nj_prime / 2, u0 +  sumbeta2)
